...
module.exports = function checkin({currentState,currentScore,bestState,bestScore,currentTemp,runElapsed,runVariations,roundNumber,roundElapsed,roundVariations}={}) {
    ...
    if (roundNumber) header += `  round:${short(roundNumber)}  ${short(runVariations)} variations in ${elapsed} (${short(runVariations/runElapsed)}/s)  ${short(currentTemp,2)}°`
    ...
    for (const name in currentScore.scores) {
        let line = `  ${name.padEnd(nameLength, ' ')} : ${short(currentScore.scores[name],2)}`
        if (bestScore) line = elide(line, currentStateWidth) + ` | ${short(bestScore.scores[name],2)}`
        console.log(line)
    }
