module.exports = function ({
	...
	acceptEqualScores,       // if truthy, equal-score variations are auto-accepted as the new "current"
	                         // state (default: only strictly-better variations bypass the temperature roll).
	                         // Useful for scenarios with many score plateaus where the optimizer needs to
	                         // wander among equal-score neighbors before finding an improvement.
	...
	do {
		...
		if (status.roundElapsed>=restartAfterTime || status.roundVariations>=restartAfterVariations || (status.bestScore.score<restartAfterScore && status.currentScore.score>=restartAfterScore) || (status.currentTemp<=restartAfterTemperature)) {
			...
			let useThisState = acceptEqualScores ? deltaScore<=0 : deltaScore<0
			if (!useThisState) {
				if (!restartAfterTemperature) status.currentTemp = tempStart / (tempScale * (tempBasedOnTime ? status.roundElapsed : status.roundVariations) + 1)
